...
fun generateAppBuildGradle(config: ProjectConfig): String = buildString {
    ...
    appendLine("    debugImplementation(libs.androidx.ui.tooling)")
    appendLine("    debugImplementation(libs.androidx.ui.test.manifest)")
    appendLine()
    appendLine("    // Dependency Injection (Hilt)")
    ...
    if (config.useRetrofit) {
        ...
        appendLine("    implementation(libs.jakewharton.serialization)")
        appendLine()
    }
    appendLine("    // Serialization")
    ...
    if (config.useRoom) {
        ...
        appendLine("    implementation(libs.androidx.room.runtime)")
        appendLine("    implementation(libs.androidx.room.ktx)")
        appendLine("    ksp(libs.androidx.room.compiler)")
        appendLine()
    }
    if (config.useDataStore) {
        appendLine("    // DataStore")
        appendLine("    implementation(libs.androidx.datastore.preferences)")
        appendLine()
    }
    appendLine("    // Testing")
    appendLine("    testImplementation(libs.junit)")
    appendLine("    testImplementation(libs.test.mockito.kotlin)")
    appendLine("    testImplementation(libs.test.kotlinx.coroutines)")
    appendLine("    testImplementation(libs.test.turbine)")
    appendLine()
    ...
    appendLine("    androidTestImplementation(libs.androidx.ui.test.junit4)")
...
fun generateVersionCatalog(config: ProjectConfig): String = buildString {
    appendLine("[versions]")
    appendLine("# Android & Kotlin Core")
    appendLine("agpVersion = \"8.13.0\"")
    appendLine("kotlinVersion = \"2.1.0\"")
    appendLine("kspVersion = \"2.1.0-1.0.29\"")
    appendLine()
    appendLine("# AndroidX Core")
    appendLine("coreKtxVersion = \"1.16.0\"")
    appendLine("appcompatVersion = \"1.7.1\"")
    appendLine("activityComposeVersion = \"1.10.1\"")
    appendLine("lifecycleRuntimeKtxVersion = \"2.9.1\"")
    if (config.useDataStore) {
        appendLine("datastorePreferencesVersion = \"1.1.7\"")
    }
    appendLine()
    appendLine("# Compose")
    appendLine("composeBomVersion = \"2025.06.01\"")
    appendLine()
    appendLine("# Dependency Injection (Hilt)")
    appendLine("hiltVersion = \"2.55\"")
    appendLine()
    if (config.useRetrofit) {
        appendLine("# Network")
        appendLine("retrofitVersion = \"3.0.0\"")
        appendLine("okhttpVersion = \"5.0.0\"")
        appendLine("jakewhartonSerializationVersion = \"1.0.0\"")
        appendLine()
    }
    appendLine("# Serialization")
    appendLine("kotlinxSerializationJsonVersion = \"1.7.3\"")
    appendLine()
    appendLine("# Coroutines")
    appendLine("coroutinesVersion = \"1.9.0\"")
    appendLine()
    if (config.useRoom) {
        appendLine("# Database (Room)")
        appendLine("roomVersion = \"2.6.1\"")
        appendLine()
    }
    if (config.useKtlint || config.useDetekt) {
        appendLine("# Code Quality")
        if (config.useKtlint) {
            appendLine("ktlintVersion = \"12.1.2\"")
        }
        if (config.useDetekt) {
            appendLine("detektVersion = \"1.23.7\"")
        }
        appendLine()
    }
    appendLine("# Testing")
    appendLine("junitVersion = \"4.13.2\"")
    appendLine("androidxJunitVersion = \"1.2.1\"")
    appendLine("espressoCoreVersion = \"3.6.1\"")
    appendLine("mockitoKotlinVersion = \"5.4.0\"")
    appendLine("turbineVersion = \"1.2.0\"")
    appendLine("uiautomatorVersion = \"2.3.0\"")
    appendLine()
    appendLine()
    appendLine("[libraries]")
    appendLine("# ==========================================AndroidX Core========================================")
    appendLine("androidx-core-ktx = { group = \"androidx.core\", name = \"core-ktx\", version.ref = \"coreKtxVersion\" }")
    appendLine("androidx-appcompat = { group = \"androidx.appcompat\", name = \"appcompat\", version.ref = \"appcompatVersion\" }")
    appendLine("androidx-activity-compose = { group = \"androidx.activity\", name = \"activity-compose\", version.ref = \"activityComposeVersion\" }")
    appendLine("androidx-lifecycle-viewmodel-ktx = { group = \"androidx.lifecycle\", name = \"lifecycle-viewmodel-ktx\", version.ref = \"lifecycleRuntimeKtxVersion\" }")
    appendLine("androidx-lifecycle-runtime-ktx = { group = \"androidx.lifecycle\", name = \"lifecycle-runtime-ktx\", version.ref = \"lifecycleRuntimeKtxVersion\" }")
    if (config.useDataStore) {
        appendLine("androidx-datastore-preferences = { module = \"androidx.datastore:datastore-preferences\", version.ref = \"datastorePreferencesVersion\" }")
    }
    appendLine()
    appendLine("# =============================================Compose===========================================")
    appendLine("androidx-compose-bom = { group = \"androidx.compose\", name = \"compose-bom\", version.ref = \"composeBomVersion\" }")
    appendLine("androidx-ui = { group = \"androidx.compose.ui\", name = \"ui\" }")
    appendLine("androidx-ui-graphics = { group = \"androidx.compose.ui\", name = \"ui-graphics\" }")
    appendLine("androidx-ui-tooling = { group = \"androidx.compose.ui\", name = \"ui-tooling\" }")
    appendLine("androidx-ui-tooling-preview = { group = \"androidx.compose.ui\", name = \"ui-tooling-preview\" }")
    appendLine("androidx-ui-test-manifest = { group = \"androidx.compose.ui\", name = \"ui-test-manifest\" }")
    appendLine("androidx-ui-test-junit4 = { group = \"androidx.compose.ui\", name = \"ui-test-junit4\" }")
    appendLine("androidx-material3 = { group = \"androidx.compose.material3\", name = \"material3\" }")
    appendLine()
    appendLine("# ======================================Dependency Injection (Hilt)==============================")
    appendLine("hilt-android = { group = \"com.google.dagger\", name = \"hilt-android\", version.ref = \"hiltVersion\" }")
    appendLine("hilt-compiler = { group = \"com.google.dagger\", name = \"hilt-compiler\", version.ref = \"hiltVersion\" }")
    appendLine()
    if (config.useRetrofit) {
        appendLine("# =============================================Network===========================================")
        appendLine("retrofit = { module = \"com.squareup.retrofit2:retrofit\", version.ref = \"retrofitVersion\" }")
        appendLine("okhttp = { module = \"com.squareup.okhttp3:okhttp\", version.ref = \"okhttpVersion\" }")
        appendLine("okhttp-logging = { module = \"com.squareup.okhttp3:logging-interceptor\", version.ref = \"okhttpVersion\" }")
        appendLine("jakewharton-serialization = { module = \"com.jakewharton.retrofit:retrofit2-kotlinx-serialization-converter\", version.ref = \"jakewhartonSerializationVersion\" }")
        appendLine()
    }
    appendLine("# ==========================================Serialization========================================")
    appendLine("kotlinx-serialization-json = { module = \"org.jetbrains.kotlinx:kotlinx-serialization-json\", version.ref = \"kotlinxSerializationJsonVersion\" }")
    appendLine()
    appendLine("# ============================================Coroutines=========================================")
    appendLine("kotlinx-coroutines-core = { module = \"org.jetbrains.kotlinx:kotlinx-coroutines-core\", version.ref = \"coroutinesVersion\" }")
    appendLine("kotlinx-coroutines-android = { module = \"org.jetbrains.kotlinx:kotlinx-coroutines-android\", version.ref = \"coroutinesVersion\" }")
    appendLine()
    if (config.useRoom) {
        appendLine("# =========================================Database (Room)=======================================")
        appendLine("androidx-room-runtime = { module = \"androidx.room:room-runtime\", version.ref = \"roomVersion\" }")
        appendLine("androidx-room-compiler = { module = \"androidx.room:room-compiler\", version.ref = \"roomVersion\" }")
        appendLine("androidx-room-ktx = { module = \"androidx.room:room-ktx\", version.ref = \"roomVersion\" }")
        appendLine()
    }
    appendLine("# ==============================================Testing==========================================")
    appendLine("junit = { group = \"junit\", name = \"junit\", version.ref = \"junitVersion\" }")
    appendLine("androidx-junit = { group = \"androidx.test.ext\", name = \"junit\", version.ref = \"androidxJunitVersion\" }")
    appendLine("androidx-espresso-core = { group = \"androidx.test.espresso\", name = \"espresso-core\", version.ref = \"espressoCoreVersion\" }")
    appendLine("test-mockito-kotlin = { module = \"org.mockito.kotlin:mockito-kotlin\", version.ref = \"mockitoKotlinVersion\" }")
    appendLine("test-kotlinx-coroutines = { module = \"org.jetbrains.kotlinx:kotlinx-coroutines-test\", version.ref = \"coroutinesVersion\" }")
    appendLine("test-turbine = { module = \"app.cash.turbine:turbine\", version.ref = \"turbineVersion\" }")
    appendLine("androidx-uiautomator = { group = \"androidx.test.uiautomator\", name = \"uiautomator\", version.ref = \"uiautomatorVersion\" }")
    appendLine()
    appendLine("# =====================================Gradle Plugins (build-logic)==============================")
    appendLine("android-gradlePlugin = { group = \"com.android.tools.build\", name = \"gradle\", version.ref = \"agpVersion\" }")
    appendLine("compose-gradlePlugin = { module = \"org.jetbrains.kotlin:compose-compiler-gradle-plugin\", version.ref = \"kotlinVersion\" }")
    appendLine("kotlin-gradlePlugin = { group = \"org.jetbrains.kotlin\", name = \"kotlin-gradle-plugin\", version.ref = \"kotlinVersion\" }")
    appendLine()
    appendLine()
    appendLine("[plugins]")
    appendLine("# =====================================Android & Kotlin Core=====================================")
    appendLine("android-application = { id = \"com.android.application\", version.ref = \"agpVersion\" }")
    appendLine("kotlin-android = { id = \"org.jetbrains.kotlin.android\", version.ref = \"kotlinVersion\" }")
    ...
    if (config.useKtlint || config.useDetekt) {
        appendLine("# ============================================Code Quality=======================================")
        if (config.useKtlint) {
            appendLine("ktlint = { id = \"org.jlleitschuh.gradle.ktlint\", version.ref = \"ktlintVersion\" }")
        }
        if (config.useDetekt) {
            appendLine("detekt = { id = \"io.gitlab.arturbosch.detekt\", version.ref = \"detektVersion\" }")
        }
        appendLine()
    }
    appendLine("# ====================================Convention Plugins (Project)===============================")
